...
### Stage 2.33 — Administrator-only operations overview (2026-08-20)

- **Objective:** make the administrator role visibly and operationally distinct from moderators without linking staff identities to pseudonymous citizen profiles. Administrators now receive a protected operations overview at `/admin`; moderators retain the pending/flagged moderation queue and audited content actions.
- **Protected content inventory:** added `GET /api/v1/community/admin/content`, authorized through `CurrentAdmin`, returning up to the 200 most recent reports and comments across pending, flagged, published, hidden, and archived states. Responses carry English/Telugu content fields and the `community_reported` classification, expose pseudonyms only, and omit district or precise-location data. No database migration was required.
- **Administrator web experience:** `/admin` now shows an explicit administrator role panel, content-state counts, an all-status filterable community-content inventory, the internal staff directory, and the ten most recent audit actions. Admin-created moderator accounts and moderation transitions refresh the relevant protected views. Moderator sessions do not request or render administrator-only inventory or staff data.
- **Security and governance:** the citizen `/account` surface remains separate and anonymous. Existing server-side `CurrentAdmin` enforcement protects both the new inventory and the staff directory; moderation transitions remain transactional and audited. The public audit log continues to hide staff email and internal identity. README, moderation policy, and deployment checks document the distinction.
- **Tests:** added a web component test covering hidden bilingual content, staff visibility, audit visibility, and status-filter selection; added an API test confirming the all-status inventory is unavailable without a staff database/session.
- **Verification evidence:** `npm run format:check`, `npm run lint`, and `npm run typecheck` passed; web Vitest passed **160 tests across 46 files**; `npm run build` succeeded with `/admin` dynamic. API Ruff passed, strict mypy passed across **117 source files**, and Pytest passed **153 tests** with **16 PostgreSQL/PostGIS integration tests skipped** because no disposable test database was configured. Focused admin tests passed 2/2 and focused community API tests passed 5/5.
- **Remaining risks:** the inventory is intentionally capped at 200 recent records and needs server-side pagination before high-volume launch. MFA, staff disable/reset controls, appeals, evidence quarantine/redaction, distributed rate limiting, and governed escalation workflows remain launch gates. Production authorization must be checked with separate administrator and moderator sessions after deployment.

### Stage 2.34 — Fail-closed community submission persistence and pending-content privacy (2026-08-20)

- **Incident found during live verification:** a submitted observation appeared immediately on the citizen `/community` page while the administrator inventory remained empty. The report insert used a generated foreign key when the hard-coded pseudonym did not yet exist, PostgreSQL rejected it, the API silently stored the report in process memory, and both the browser submit handler and public fallback list rendered the unapproved record.
- **Persistence fix:** report and comment writes now create the missing pseudonymous `UserAccount` in the same transaction before inserting `pending_review` content. The implicit profile stores no district, defaults both broad data-sharing consent flags to false, and remains separate from staff identities. Citizen-profile, report, and comment writes now return 503 in `APP_ENV=production` when PostgreSQL is unavailable or rejects the transaction; production paths never silently create process-local records. No schema migration was required.
- **Publication boundary:** development fallback report/comment lists now explicitly filter to `published`. The citizen web client no longer prepends a newly submitted pending report/comment into the public list; it instead displays bilingual English/Telugu confirmation that moderator approval is required.
- **Regression coverage:** API tests assert pending fallback reports and comments are absent from public lists and that a production submission without persistence fails closed. The community UI test submits an observation, verifies the moderator-review confirmation, and proves the pending title is not rendered publicly.
